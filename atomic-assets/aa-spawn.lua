...
-- if Name ~= 'AchieveingHope' then Name = 'AchieveingHope' end
...
-- ProcessId: nQTO3xDx8sjD67QAgN6AP7rf8j-aoTK17emhx-bJo1g
...
function rejectRequest(target)
    local schema = json.encode({
        BuySuperLlamas = {
            Title = "hey fuckerrr!!",
            Description =
            "You don't have ANY Ownership over me so fo!",
            Schema = nil
        }
    })
    Send({
...
function checkBalance(sender)
    if Balances[sender] and tonumber(Balances[sender]) > 0 then
        return true
    else
        return false
    end
...
Handlers.add('Schema', Handlers.utils.hasMatchingTag('Action', 'Schema'),
    function(msg)
        if checkBalance(msg.From) then
            showOptions(msg.From)
        else
            rejectRequest(msg.From)
        end
    end)
